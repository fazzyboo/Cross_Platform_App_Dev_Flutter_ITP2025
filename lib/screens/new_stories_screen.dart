...
class NewStoriesScreen extends ConsumerWidget {
  const NewStoriesScreen({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final AsyncValue<List<NewsItem>> newStories = ref.watch(newStoriesProvider);

    return Scaffold(
      appBar: AppBar(
        title: const Text('Hacker News New Stories'),
      ),
      body: newStories.when(
        data: (stories) {
          if (stories.isEmpty) {
